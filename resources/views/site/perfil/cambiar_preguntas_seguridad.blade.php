...
@section('content')
    <div class="bg-slate-50 flex items-center justify-center p-4">
        <div class="max-w-6xl w-full bg-white rounded-[3rem] overflow-hidden border border-slate-400">
            <div class="flex flex-col lg:flex-row">
                <div class="lg:w-1/3 bg-slate-50 p-8 md:p-12 border-b lg:border-b-0 lg:border-r border-slate-200">
                    ...
                    <div class="space-y-6">
                        @forelse($user->respuestasSeguridad as $index => $respuesta)
                            <div class="relative pl-6 border-l-2 {{ $index == 0 ? 'border-blue-200' : 'border-slate-200' }}">
                                <span class="absolute -left-[9px] top-0 w-4 h-4 {{ $index == 0 ? 'bg-blue-500' : 'bg-slate-300' }} rounded-full border-4 border-slate-50"></span>
                                <p class="text-[10px] font-black {{ $index == 0 ? 'text-blue-600' : 'text-slate-400' }} uppercase tracking-widest mb-1">
                                    Pregunta {{ $index + 1 }}
                                </p>
                        ...
                        @empty
                            <div class="p-4 bg-blue-50 border border-blue-100 rounded-2xl mb-3">
                                <p class="text-xs text-blue-600 font-medium italic">Aún no has configurado tus preguntas.</p>
                            </div>
                        @endforelse
                    </div>
                ...
                <div class="lg:w-2/3 p-8 md:p-12">
                    ...
                    @if (session('success'))
                        <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 text-emerald-600 rounded-2xl text-sm font-bold animate-pulse">
                            ✅ {{ session('success') }}
                        </div>
                    @endif
